...
class CanonicalIntersectionObserver {
  ...
  observe(element, options) {
    let isObserving = _lodash.default.some(this._cache, function (cacheEntry) {
  ...
  unobserve(element, options) {
    ...
    _lodash.default.forEach(cache, cacheEntry => {
      let {
        element,
        observer } =
      cacheEntry;
  ...
  disconnect() {
    ...
    _lodash.default.forEach(observers, observer => {
      observer.disconnect();
