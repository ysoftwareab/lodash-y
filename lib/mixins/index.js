...
module.exports = { "abstract": function (name = 'it') {return function () {throw new Error(`Calling an abstract function. Please implement ${name}.`);};}, "alwaysPromise": function (maybePromiseLike) {// eslint-disable-next-line consistent-this, no-invalid-this
    ...
    // eslint-disable-next-line no-console
    console.timeEnd(label);return returnValue;}, "defer": function () {let deferred = {};deferred.promise = new Promise(function (resolve, reject) {deferred.resolve = resolve;deferred.reject = reject;});return deferred;}, "safeProxy": function (env) {
    ...
    return new Proxy(env, { get: function (target, property, _receiver) {if (property === '_') {return target;}if (!_.isString(target[property])) {throw new Error(`${property} is undefined.`);}return target[property];} });}, "mapValuesDeep": function (fn) {
